---
title: Miscellaneous
---

# Skills
- **Languages:**  
**Chinese *(Mandarin native speaker)***, **English *(IELTS 6.5/L7/R7.5/W6/S6)***, some other East-Asian languages.

- **Programming:**  
**Python**, **C/C++**, **Linux Shell**, C#, Java(Android), Verilog, MIPS ASM, enthusiastic to discover more!

- **Data Mining Tools:**  
**Scikit-learn**, **Pandas**, **Numpy**, **Matplotlib**, PyTorch, clustering algorithm, LSTM autoencoder.

- **Embedded Systems:**  
**OpenWRT**, **microcontroller programming**, **bootloader hacking**, Bluetooth, LoRA, PCB design.

- **OS & Utilities:**  
**Linux**, **QEMU**, **iptables**, cross compiling, Git, DPDK.

- **Android OS:**  
ROM modifying, ADB debugging, Monkeyrunner, reverse engineering, API hooking.

# Honors and Awards

- **Nov. 2021** - Offer from Xiaomi Technologies on Android Developing.
- **Nov. 2019** - 1st class university scholarship in 2019, and other classes in 2016, 2018, 2020 and 2021.
- **Sep. 2017** - Finalist prize in Loongson Cup 1st National Student Computer System Capability Challenge.
- **Nov. 2016** - 3rd prize in 14th Shandong Provincial Software Design Competition for University Students.
- **Sep. 2016** - Finalist prize in Inspur Cup 7th Shandong Provincial ACM-ICPC Programming Competition. 
- **Sep. 2013** - 3rd prize in National Olympiad in Informatics in Provinces. 


# Volunteering

- **Aug 2022** - Volunteering member @ [ICIVIS 2022](http://www.icivis.net/), Jinan, China.
- **Jul 2021** - Volunteering member @ [ACM TURC 2021](https://www.acmturc.com/2021/cn/index.html), Hefei, China.
- **Nov 2020** - Volunteering member @ ICOC 2021, Jinan, China.
- **Oct 2020** - Volunteering member @ CNCC 2020 (China National Computing Congress), Jinan, China.
- **2011-2013 - Early member of [Anthon Open-Source Community](https://aosc.io).**